...
// https://github.com/nix-community/infra/blob/master/hosts/build02/filter.sed
// Commit: 1416f0bad404696b6cdeb7db2da6770abb3da2ac
// Note that the regexes that just drop packages entirely don't need to be
// implemented, as subscribing to a failed update of a package that never gets
// updated automatically does not break anything.
...
func NormalizeAttrPath(ap string) string {
	for _, n := range normalizations {
		if loc := n.pattern.FindStringIndex(ap); loc != nil {
			return ap[:loc[0]] + n.replacement + ap[loc[1]:]
		}
	}
	return ap
}
